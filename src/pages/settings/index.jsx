...
//   return (
//     <div>
//       <style>{`input:focus { border-color: #6366F1 !important; outline: none; }`}</style>
...
export default function Settings() {
  ...
  const [editAvatar, setEditAvatar] = useState(false);
  const [avatarColor, setAvatarColor] = useState("#BFDBFE");
  const [showProfileCard, setShowProfileCard] = useState(false); // New state for toggling profile card
  ...
  return (
    <div>
      ...
      <div className="shadow-sm bg-white p-5 border border-[#E0E0E0] rounded-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {/* Button to show profile card */}
          <div>
            <div className="text-center">
              <button
                onClick={() => setShowProfileCard(!showProfileCard)}
                className="w-full max-w-62 bg-linear-to-r from-[#1D3557] to-[#2C4B7C] text-white border-none rounded-2xl sm:rounded-[20px] md:rounded-3xl lg:rounded-[27px] py-2 px-3 sm:py-3 sm:px-4 md:py-4 md:px-5 lg:py-4 lg:px-6 font-bold text-[12px] sm:text-[14px] md:text-[15px] lg:text-[16px] cursor-pointer shadow-lg hover:shadow-xl transition-all duration-300"
              >
                {showProfileCard
            ...
            {/* Profile Card - only shown when showProfileCard is true */}
            {showProfileCard && (
              <div className="mt-5 bg-white rounded-[20px] shadow-[0_2px_16px_rgba(0,0,0,0.07)] p-6 sm:p-7 md:p-8 animate-fadeIn">
                {/* Avatar + Info */}
                <div className="flex flex-col sm:flex-row items-start gap-4 mb-6">
                  <div
                    className="self-center sm:self-auto"
