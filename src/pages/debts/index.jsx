...
const Field = ({ label, children }) => (
  <div style={{ marginBottom: 14 }}>
    <label className="text-[12px] text-gray-500 font-semibold uppercase tracking-[0.8px] block mb-1.75">
      {label}
    </label>
    {children}
  </div>
...
function Modal({ onClose, onSave, editItem, today, defaultType, loading }) {
  const [form, setForm] = useState(
    editItem
      ? {
          ...
          date: editItem.date ? editItem.date.slice(0, 10) : today,
          due_date: editItem.due_date ? editItem.due_date.slice(0, 10) : today,
          note: editItem.note || "",
        }
      : {
          person_name: "",
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-[clamp(14px,2.5vw,19px)] mb-5 font-bold">
          {editItem
            ? "Edit Record"
            : form.kind === "I owe"
              ? "+ Add debt"
  ...
  return (
    <div>
      ...
      <div className="shadow-sm bg-white p-3 sm:p-5 border border-[#E0E0E0] rounded-2xl">
        {/* Header */}
        <div className="action-buttons flex justify-between items-center mb-5.5 gap-4 flex-nowrap w-full">
          {/* Sarlavha - flex-1 va truncate sarlavha ko'p joy egallab tugmalarni siqib qo'ymasligi uchun */}
          <h3 className="text-[clamp(16px,4vw,24px)] font-bold text-[#1F2937] flex-1 truncate mr-4">
            Total Debts and Receivables
          </h3>

          {/* Tugmalar konteyneri - shrink-0 bitta qatorda qotib turishini ta'minlaydi */}
          <div className="flex gap-2.5 items-center shrink-0">
            {/* Add Receivable Tugmasi */}
            <button
              onClick={() => setModal("receivable")}
              disabled={loading}
              className={`flex items-center justify-center bg-[#10B981] text-white rounded-[11px] w-10 h-10 md:w-auto md:h-auto md:min-w-42.5 px-3.75 md:px-4.5 md:py-2.5 font-bold text-[13px] whitespace-nowrap transition-all
        ${loading ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:brightness-95 active:scale-95"}
      `}
            >
              <span className="text-xl leading-none">+</span>
              <span className="hidden md:inline ml-2 text-[13px]">
                Add Receivable
              </span>
            </button>
            ...
            <button
              ...
              disabled={loading}
              className={`flex items-center justify-center bg-[#EF4444] text-white rounded-[11px] w-10 h-10  md:w-auto md:h-auto md:min-w-42.5 px-3.75 md:px-4.5 md:py-2.5 font-bold text-[13px] whitespace-nowrap transition-all
        ${loading ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:brightness-95 active:scale-95"}
      `}
            >
              <span className="text-xl leading-none">+</span>
