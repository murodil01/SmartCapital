...
function Btn({ onClick, children, variant = "primary", style = {}, loading }) {
  const base = {
    flex: 1,
    border: "none",
    borderRadius: 8,
...
export default function Family() {
  ...
  const saveEdit = async () => {
    setModalLoading(true);
    try {
      await familyAPI.update(editId, editForm);
      setMembers((prev) =>
        prev.map((m) =>
          m.id === editId
            ? {
      ...
      return (
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            ...
            disabled={page === totalPages || modalLoading}
          >
            ›
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div style={s.page}>
        ...
        <Modal
          show={inviteModal}
          onClose={() => !modalLoading && setInviteModal(false)}
        ...
        <Modal
          show={editModal}
          onClose={() => !modalLoading && setEditModal(false)}
          title="Edit Member"
        >
          <Field label="Full Name">
            <input
        ...
        <Modal
          show={goalModal}
          onClose={() => !modalLoading && setGoalModal(false)}
